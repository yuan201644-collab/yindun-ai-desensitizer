...
SENSITIVE_PATTERNS = {
    ...
    "签发机关": {
        "pattern": r"(?:签发机关|机关)[:：\s]*([\u4e00-\u9fa5]{2,15})",
        "category": "identity",
        "risk_level": "medium",
        "group": 1,
    },
    "有效期限": {
        "pattern": r"(?:有效期限|有效期)[:：\s]*([0-9]{4}\s*[./-]\s*[0-9]{1,2}\s*[./-]\s*[0-9]{1,2}[^\n]{0,12}|长期)",
        "category": "identity",
        "risk_level": "medium",
        "group": 1,
    },
    ...
    "家庭住址": {
        ...
        #   机关负向前瞻（公安局/法院等）排除「泗洪县公安局」类误报；「(?<!城)市」排除「城市」；
        #   group=1 → 敏感值从「江苏」等行政区划名开始，标签（住址/地址）保留
        "pattern": r"(?:家庭住址|常住地址|现住地|住址|地址|户籍|籍贯)?((?:[\u4e00-\u9fa5]{1,4}(?:省|(?<!城)市|县|镇|乡|村|路|街|道|巷)(?!机关|公安局|法院|检察院|税务局|委员会|办公室|派出所|人民政府)|[\u4e00-\u9fa5]{2,6}(?:区|栋|室|楼|单元))[\u4e00-\u9fa5\d]{0,20})",
        "category": "location",
        "risk_level": "high",
        "group": 1,
    },
    ...
    "QQ号": {
        # ⭐ 上下文约束（QQ/扣扣/企鹅 前缀）——裸数字不再误标（邮编/热线/日期等 5-11 位数字）；
        #   group=1 → 只打码号码，「QQ：」标签保留
        "pattern": r"(?:QQ|扣扣|企鹅|qq)[:：号\s]*([1-9]\d{4,10})",
        "category": "social",
        "risk_level": "low",
        "group": 1,
    },
